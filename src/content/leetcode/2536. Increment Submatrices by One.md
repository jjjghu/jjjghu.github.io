...
title: "2536. 子矩陣元素加 1"
en_title: "2536. Increment Submatrices by One"
...
en_link: "https://leetcode.com/problems/increment-submatrices-by-one/"
date: "2025-11-14"
---
## 思路
二維差分數組的經典題目。
...
差分數組的好處在於，能將區域遞增固定數值 $O(n)$ 操作轉化為 $O(1)$ 操作。\
假設想要讓$[2, 5, 1]$同時增加 10，數組會變成 $[1, 12, 15, 11, 7]$，差分數組會變成$[1, 11, 3, -4, -4]$\
跟原本的差分數組相比，在`2`所在的位置，差分數組數值增加 10，在`7`所在的位置減去10，\
也就是說，想要將`nums[i] ~ nums[j]`的數字統一增加`x`，\
在差分數組中，只要將`diff[i] += x`，`diff[j + 1] -= x`就可以了。
