...
1. 接下來要嘗試能不能合成出 $10000$，我們用哈希表將出現過的數字紀錄下來。\
    並在遍歷時判斷目標數字是否曾出現在哈希表中，可以合成出來，那麼最後答案的最高位就是$1$。
...
```
### 思路2. 前綴樹
#### [前綴樹](https://gjplieqszy7.sg.larksuite.com/wiki/KJC1wRJAqidzAqkpCEClyp2YgUf#share-Pwxadll2ioLoZgxOSdUltPvFgTc)
假如當前要配對的數字是 $a=99=01100011$，想讓結果數值大，得盡量讓前面位數跟配對的數字相異。\
因此，把所有數丟到前綴樹中後，拿數字 $a$ 到前綴樹中去找。
