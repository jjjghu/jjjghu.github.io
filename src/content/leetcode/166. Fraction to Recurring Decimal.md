...
title: "166. 分數到小數"
en_title: "166. Fraction to Recurring Decimal"
...
en_link: "https://leetcode.com/problems/fraction-to-recurring-decimal"
date: "2025-09-24"
---
## 思路
先把整數部分和符號算出，剩下的部分模擬直式除法處理即可。\
兩數的範圍都在整個 $\text{int}$ 的範圍當中，相除可能會溢出，因此要轉為 $\text{long long}$。
...
```
## 複雜度分析
- 空間複雜度：$O(n)$，$n$為「可能出現的餘數有幾種」，因此最多會有 $\text{denominator}$ 種，存在哈希表裡面。
- 時間複雜度：$O(n)$
