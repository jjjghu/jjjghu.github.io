...
## 思路
因為每個鏈表都是有序的，跟合併排序一樣，不過現在有 k 個指針，每次需要找到這些數字當中的最小值。\
每次找到最小值之後，將對應鏈表的下一個數值加入到參考當中，然後再找一次最小值。\
這樣就能排序好這些數字了。

---

問題在於，要怎麼快速找到「一堆數字當中的最小值」，並且在加入新數字之後，依舊能快速取出最小值？\
...
public:
    ListNode* mergeKLists(vector<ListNode*>& lists) {
        ...
        for(auto node : lists) { 
            if(node != nullptr) { // 把每個鏈表的第一個節點加到 pq 當中
                pq.push(node);
            }
        }
