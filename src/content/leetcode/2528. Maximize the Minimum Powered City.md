...
title: "2528. 最大化城市的最小電量"
en_title: "2528. Maximize the Minimum Powered City"
...
en_link: "https://leetcode.com/problems/maximize-the-minimum-powered-city/"
date: "2025-11-07"
---
...
首先要計算出每個城市現有的電量`energy[i]`是多少，\
這可以用前綴和或是滑動窗口做到，將複雜度從 $O(n^2)$ 降低到 $O(n)$。
...
因為遍歷時是從前面往後，表示在`i`之前的城市電力都已經足夠，\
...
```
## 複雜度分析
- 時間複雜度：$O(n\log{k})$, `n`是`station`的長度，二分查找$O(\log{k})$
- 空間複雜度：$O(n)$
